...
class AlmPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('alm.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `alm`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM alm WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO alm (date, title, link, content)"
